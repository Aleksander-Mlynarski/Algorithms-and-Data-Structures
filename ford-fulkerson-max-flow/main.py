...
if __name__ == '__main__':
    ...
    lista_grafow = [graf_0, graf_1, graf_2, graf_3]
    starty = ['s', 's', 's', 's']
    konce = ['t', 't', 't', 't']
    wezly_testowe = ['u', 'a', 'a', 'a']
    
    for i in range(4):
        obecny_graf = lista_grafow[i]
        start = starty[i]
        end = konce[i]
        test_node = wezly_testowe[i]
        
        g = Lista_sasiedztwa()
        
        for j in range(len(obecny_graf)):
            v1 = obecny_graf[j][0]
            v2 = obecny_graf[j][1]
            cap = obecny_graf[j][2]

            edge_fwd = Edge(cap, False)
            g.insert_edge(v1, v2, edge_fwd)
            
            edge_rev = Edge(cap, True)
            g.insert_edge(v2, v1, edge_rev)

        wynik = g.FF(start, end)
        print(wynik)
        ...
        if (test_node in g.nodes):
            for sasiad, krawedz in g.neighbours(test_node):
                if (krawedz.flag == False):
                    outflow = outflow + krawedz.flow
